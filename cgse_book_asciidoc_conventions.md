...
- **Environment issue, resolved 2026-08-14:** `:front-cover-image:` used to crash `asciidoctor-pdf` 2.3.10 on this machine's old system Ruby (2.6.10, `undefined method 'absolute_path?' for File:Class` — needs Ruby ≥ 2.7). Fixed by installing a modern Ruby via Homebrew and reinstalling `asciidoctor`/`asciidoctor-pdf`/`asciidoctor-tabs`/`rouge` under it — gems are per-Ruby-install, so a Ruby upgrade alone isn't enough. `:front-cover-image:` is enabled and confirmed rendering. Note for future diagnosis: a non-interactive shell may not pick up a `.zshrc` `PATH` change the way an interactive terminal does; if `ruby -v` still shows the old version after an upgrade in such a shell, check `gem environment`'s `EXECUTABLE DIRECTORY` and invoke by full path rather than assuming the upgrade failed.
