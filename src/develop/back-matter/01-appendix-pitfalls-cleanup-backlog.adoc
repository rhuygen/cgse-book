...
Issue:: Both functions apply essentially the same regex (`SETUP_(\w+)_([\d]{5})_([\d]{6})_([\d]{6})\.yaml`) against a Setup filename, one returning just the `setup_id`, the other returning `(site_id, setup_id)`. A single parsing function returning a small dataclass/namedtuple (or `None`) would remove the duplication and the risk of the two regexes drifting apart if the filename convention ever changes.
...
Fix scope:: Medium. A cheap improvement: log a warning when `recursive_dict_update` is about to overwrite an existing top-level group key from a _different_ entry-point than the one that first defined it. Doesn't need to become a hard error — just needs to stop being silent.
...
_Leftover "do we still use these" note._
...
_Confirmed against three sibling classes._
...
_Self-flagged in source with a FIXME comment._
...
_Unclear relationship, acknowledged TODO in source._
...
Update (Chapter 8):: The *relationship* question is resolved, with source evidence: ``egse/protocol.py``'s `BaseCommandProtocol.send_commands()` docstring states outright that `CommandProtocol` pairs with `+@dynamic_interface+`-decorated commands (needs the YAML-loaded dictionary shipped to the client) while `DynamicCommandProtocol` pairs with `+@dynamic_command+`-decorated commands (nothing to ship — the decorator puts everything the client needs directly on the method). `Proxy`/`CommandProtocol`/`+@dynamic_interface+` and `DynamicProxy`/`DynamicCommandProtocol`/`+@dynamic_command+` are two complete, intentionally coexisting systems — the second was built specifically to remove the "control server must already be running when the `Proxy` is constructed" constraint the first system carries (Chapter 8 walks through both, plus a real before/after pair, PUNA vs. PUNA+, and a device driver — `daq6510.py` — that deliberately mixes both decorators on one interface class). This does *not* resolve the other two items logged under this entry — the `+# TODO (rik): remove all methods from Proxy...+` comment and the `get_service_proxy()` FIXME are `Proxy`-specific cleanup, unrelated to `DynamicProxy`, and remain open. Worth mirroring this "why both exist" explanation as a short cross-reference note directly in the source (on both `Proxy` and `DynamicProxy`), so a future reader doesn't have to find this book to get the same answer.
...
_Confirmed by reading the actual `entry_points()` call sites._
...
_Confirmed by cross-checking every workspace member's declared dependencies against the root table._
...
=== Mixin and Dynamics Chapter (Chapter 8)

==== P-012 `CommandType` Enum Is Missing a `QUERY` Member

_Confirmed by grepping every `cmd_type=` call site across the monorepo._

Module:: `egse/mixin.py`
Where:: `class CommandType(str, enum.Enum)`
Issue:: `CommandType` — the typed alternative to a bare string for ``dynamic_command()``'s `+cmd_type+` argument — defines `READ`, `WRITE`, and `TRANSACTION`, but no `QUERY`, even though `+"query"+` is a fully valid, commonly used `+cmd_type+` (`COMMAND_TYPES` recognizes it, and `egse/hexapod/symetrie/dynalpha.py` uses `+cmd_type="query"+` throughout the PUNA+ hexapod driver). Any device driver wanting a `CommandType`-typed query command has no member to reach for and has to fall back to the raw string, inconsistently with the other three command types.
Fix scope:: Trivial — add `QUERY = "query"` to the enum.
Risk of leaving as-is:: Very low — purely a missing convenience, not a functional bug (the raw string works identically). Worth fixing mainly for consistency, since three of the four `cmd_type` values have a typed member and one doesn't, with no apparent reason for the gap.
