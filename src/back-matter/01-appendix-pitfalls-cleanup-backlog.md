# Pitfalls and Cleanup Backlog Appendix
...
## Settings and Setup Chapter (Chapter 4)
...
## env.py Chapter (Chapter 5)

### P-003 `has_conf_repo_location()` Always Returns `False`
...
### P-005 Stray Unresolved Comment in `env.py` `main()`

*Leftover "do we still use these" note.*
...
## control.py and proxy.py Chapter (Chapter 6)

### P-006 `ControlServer.service_id` Is Never Actually Set

*Confirmed against three sibling classes.*
...
### P-007 Monitoring `PUB` Socket Registered in the Poller
...
### P-008 `DynamicProxy` vs `Proxy` Relationship

*Unclear relationship, acknowledged TODO in source.*
...
### P-011 Unexplained Entries in the Root `[tool.uv.sources]` Table

*Confirmed by cross-checking every workspace member's declared dependencies against the root table.*

- **Module:** Root `pyproject.toml` (`[tool.uv.sources]`), plus the member-level `[tool.uv.sources]` tables in `libs/cgse-core/pyproject.toml`, `libs/cgse-coordinates/pyproject.toml`, and `projects/generic/symetrie-hexapod/pyproject.toml`.
- **Where:** `[tool.uv.sources]`.
- **Issue:** Two separate small inconsistencies in the same mechanism. First, per `uv`'s own workspace docs, sources declared in the root `pyproject.toml` are inherited as the default for every workspace member, and a member's own `[tool.uv.sources]` only matters if it overrides that default — but none of the three member-level tables in this workspace actually differ from what root already provides; they just restate the same `{ workspace = true }` entry root already inherits down to them, including a redundant `editable = true` on `symetrie-hexapod`'s entry for `cgse-coordinates` (workspace members are always installed editable regardless of that flag, per `uv`'s docs — it does nothing). Second, root's table lists `keithley-tempcontrol`, `lakeshore-tempcontrol`, and `symetrie-hexapod` as sources, but nothing anywhere in the workspace declares any of those three as a dependency — every other entry in the table corresponds to a real dependency edge (e.g. `cgse-gui` is needed by several device packages, `plato-spw` by `plato-fits`/`plato-hdf5`), but these three don't.
- **Fix scope:** Trivial cleanup, no functional risk either way — remove the three unused entries from root's table, and remove the three redundant member-level `[tool.uv.sources]` tables (or keep them only if a genuine future override is intended, at which point a comment explaining why would help).
- **Risk of leaving as-is:** Very low. Everything resolves correctly today; this is pure housekeeping. Worth fixing mainly so a future contributor reading `[tool.uv.sources]` doesn't waste time looking for a reason these three entries exist that isn't actually there.
